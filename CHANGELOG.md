...
- **Fedora + Debian packaging** ([setup/](setup/)). `install.sh` (generic, package-manager-aware dependency hints), `install-teacher.sh`, `uninstall.sh`, `package-deb.sh` and `package-rpm.sh`, each producing **two** packages — `classsend2` (agent + student TUI, autostart) and `classsend2-teacher` (teacher console) — for both Mint/Debian (.deb) and Fedora/RHEL (.rpm). The .deb builder falls back to `ar`+`tar` so it runs on Fedora too. Adds `xdg-utils` to the runtime deps.
